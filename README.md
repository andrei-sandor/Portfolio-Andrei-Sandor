...
- 🧑🏻‍🎓 First Year Masters student in Computer Science (Non-Thesis) at McGill University. Graduate Software Engineering Co-op student at McGill University
...
Libraries/Frameworks: 
  Scikit-learn, Pandas, NumPy, Matplotlib, Flask, UMPLE, FastAPI, Vue.js, Spring, React/NextJS, asyncio, JavaFx,audioop,SQLite, Pickle, JUnit, TensorFlow, Discord_Api, openai, joblib, repl.it db, ntlk, tabula, tabulate, PyPDF2, xarray, specklepy.api, steamlit, plotly, speckle_io, selenium, openpyxl, win32com, smtplib, forex,  seaborn

Technologies:
  Github, Git, VSCode, JupyterNotebook, Eclipse, Gradle, Hibernate, PostgresSQL, MSAzure, Streamlite, AWS, Google Cloud, GoDaddy, SQL, Google Tools, PyCharm, IntelliJ, ARMv7DE1-SoC Emulator, Postman, JSON, Gherkin, Repl.it cloud service. Anaconda, Discord, Bootstrap
...
Created in a team of 5 a programming project for a fictuous situation to generate a new tunnel in the city of Kai Tak in Hong Kong. To realize this project, we had to work with geological and GIS data of the city that is represented as a voxel (think Minecraft). Each voxel has a specific value between 0 and 1 representing the situation at that position. A value of 1 means good place to build the tunnel and a value of 0 means it is not a good place. The best tunnel has the highest product of the value of each voxel. 

To generate this tunnel, many algorithms were tried like beam algorithm, divide and conquer algorithm, viterbi algorithm and pruning algorithm. Finally, the best algorithm was beam algorithm. This was programmed by using Python with Jupyer Notebook. Numpy was used to generate a local data environemnt. Then, when the algoithm was done, we had to link it to the frontend and create an app as backend. To do this we use extensively the Python libraries specklepy.api and streamlit. The app contains the authentication section, a HTML connection, it can select a Speckle project, it can select a speckle project and it can transfer the backend to the frontend. Then, the specific data was generated from the frontend by using Python function to bring them to the backend. All the data was converted to a dictionary of pandas DataFrames to be able to better use the data in the backend. Finally, there is an option to create a stream between the frontend and the backend. We generated mock data by using advanced mathematcs, numpy and xarray. Also, we had some data anaylzing section with Python/pandas in a JupyterNotebok. For this one, we created a scroe voxel advanced function. We also had to fix a geological model by using speckle api to extract the data files to use it for the backend. We also had a translate_data section. We also used the speckle api and Pandas to generate all the voxel data of the world we working from.

Link repo: https://github.com/bedrock-systems/speckle-hackathon 
<img width="663" alt="Screenshot 2024-07-19 at 8 01 49 PM" src="https://github.com/user-attachments/assets/50087056-2983-4175-9cd2-f5c521c2b156">
...
For the backend (find the best match, the state of the engine and the maintenance), Jupyter Notebook files have been created first where we determine the best regression algorithm  (by using metrics) with the help of ML Python libraries. For these models, the csv files have been replaced by a SQL databse. Then, to link it to the frontend, we used FastApi which takes the models and create endpoints on them. Then, when those endpoints were ready, they were deployed via Google Cloud and MS Azure. Then, the frontent was done with NextJS that put all the UX/UI together. NextAuth for authentication, Google Geolocation and Directions API were used to intelligently translate input coordinates and display the directions between the two points. 
...
### The Automator (Summer 2024)
Tools:
- Bootstrap
- Flask (sqlalchemy, templating, migrate, request, redirect)
- Python
- selenium
- openpyxl
- win32com
...
### Website Andrei Sandor (Fall 2023)
...
Created alone a website showcasing main highlights of my professional and personal career. This website is composed of main tabs redirecting to diffrent links. The first tab is the main tab. This tab contains a picture of me and a quick introduction about myself. Then, the next tab is about the schools that I have went. Furthermore, there is an experience tab where I showcase my professional internship experience so far. Then, the next tab is an elegant high-level view of the main projects that I have worked so far on. The next tab is about tutoring oportunities that I offer. This tab is a form that further tutee complete where I collect all the relevent information so that I can help them. Finally, the last tab is about important links related to me.

To create this website, I used all my frontend experience from my three years of high-school programming. I applied all my HTML knowledge basing around div and advanced objects in HTML (input boxes, drop-down lists, calendar input, number and string inputs,checkboxes, lists, links, pictures). Then, to make everything look good, I applied the CSS by touching a variety of features that I know (position, margin, width, height, top,left,bottom,right, padding, text-align, box, background-color) to customize in a simple and efficient way to produce an elegent view without super advance framework. Finally, for the tutoring form, I included some JavaScript to make it more interesting. Also, I used all my school experience related to frontend from courses like ECSE 321 (Intro to Software Engineering) and ECSE 428 (Software Engineering Practice). I applied the Vue.js to create a nice header for each file. I was working around views and router. I included all my HTML, CSS and JS into view files. I also organized the website by creating the tabs and the overall structure.

Link to repo: https://github.com/andrei-sandor/WebsiteAndreiSandor

<img width="1512" alt="Screenshot 2023-12-02 at 3 16 19 PM" src="https://github.com/andrei-sandor/Portfolio-Andrei-Sandor/assets/97865484/15480fd9-9d70-4252-9ff4-76e46fbda076">
<img width="1512" alt="Screenshot 2023-12-02 at 3 16 31 PM" src="https://github.com/andrei-sandor/Portfolio-Andrei-Sandor/assets/97865484/1e476921-257c-4e34-b836-3b4160ecace1">
...
- IntelliJ IDEA
  ...
  For this game, the structure of the game is simple and efficient. There is a big while loop that handles the state of the game. Inside the while loop, each player will have a section. There will be an input taken and converted to a position on the board by working with the Arrays from java.util.Arrays. Then, there will be a check for a win or a draw. Then, the next player will have the same logic applied. There are also two functions that deal with win and draw for which an efficient method to work with multidimenstional arrays is used.
